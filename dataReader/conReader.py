...
class conReader(data.Dataset):
    ...
    def __init__(self, srcFolder, transformation=None, isTrain=True, isCV=False):
        """
        初始化数据集类，srcFolder代表数据文件夹，默认没有变换且是训练数据集
        """
        allFiles = defaultdict(list)
        files = [(name.split('_')[-3], os.path.join(srcFolder, name))
                 for name in os.listdir(srcFolder) if name[-3:] == 'jpg']  # 读入所有jpg文件
        for k, v in files:
            allFiles[k].append(v)
